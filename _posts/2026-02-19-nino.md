...
> NiNo is essentially a GNN that takes a history of past parameter values along the optimization trajectory (which can be obtained by Adam or another optimizer) and making a jump by predicting future parameter values. 
...
```plotly
{
  ...
  "layout": {
    "title": {
      "text": "NiNo achieves ~2x speedup compared to Adam.",
      "font": {
        "size": 20
      }
    },
...
Note the **~2x reduction** of the number of steps required by NiNo to achieve the same validation perplexity as by Adam.
The optimization task in this example is autoregressive next-token prediction on the wiki dataset that NiNo has not seen during its training.

## NiNo Details

As NiNo is a neural network (namely, a GNN), it needs to be trained first before we can use it to speed up optimization. 
To do so, we collected and publicly released a 🤗[dataset of checkpoints](https://huggingface.co/datasets/SamsungSAILMontreal/nino_metatrain) with optimization trajectories on 2 vision and 2 language tasks. 
Even though collecting these checkpoints and training NiNo is computationally expensive, 
this single-time cost is amortized meaning the same trained NiNo can be potentially used on millions of tasks to eventually save much more costs.
...
accurate modeling of **neuron permutation symmetry**<d-footnote>Modeling neuron permutation symmetry is imposing a strong inductive bias similarly to using
convolution for images. So our model can be used in more diverse tasks and should be able to learn parameter prediction rules that generalize better with fewer samples compared to models that do not explicitly take this symmetry into account.</d-footnote>.
Neuron permutation symmetry states that the order of neurons in adjacent layers of a neural network layer can be 
randomly permuted without affecting the overall function of the network<d-cite key="hecht1990algebraic"></d-cite>.
...
and an element-wise activation function σ. Given input $\mathbf{x}$, the output of such a network can be expressed as:
...
$$
f(\mathbf{x}) = (\mathbf{W}_2 \  P^{-1}) \ \sigma(P \ \mathbf{W}_1 \  x).
$$

Given the orthogonality property of permutation matrices, $$ \mathbf{P}^{-1}\mathbf{P} = \mathbf{I}$$, 
and that the activation function σ is element-wise, we can see that the output of the network remains unchanged:

$$
f(\mathbf{x}) = \mathbf{W}_2 \  P^{-1} \ \sigma ( \mathbf{P} \ \mathbf{W}_1 \ x) = \mathbf{W}_2 \  \mathbf{P}^{-1} \mathbf{P} \ \sigma (\mathbf{W}_1 \ x) = \mathbf{W}_2 \ \sigma(\mathbf{W}_1 \ \mathbf{x}).
$$
...
<div id="mlp-permutation-demo">
  ...
  <div style="text-align: center; margin-top: 1rem;">
    ...
    <button id="toggle-btn">✓ Correct Permutation ON</button>
  </div>
...
making it stand out compared to WNN (that ignores the neural network structure). 
...
<div class="row mt-3 l-body figure-container-desktop">
   <div class="col-sm mt-3 mt-md-0" style="text-align: center;">
        {% include figure.liquid path="assets/img/2026-02-19-nino/neural_graph_msa.png" class="img-fluid d-block mx-auto rounded z-depth-1" zoomable=true %}
    </div> 
</div>
...
## Results that Stands Out

- NiNo achieves a roughly ~2x speedup compared to Adam, which is generally unreachable by manually-designed optimizers (which are considered remarkable if a 1.2–1.3x speedup is achieved).
...
Our work connects to the broader "learning to optimize" literature, with seminal Luke Metz's et al. works such as VeLO<d-cite key="metz2022velo"></d-cite>. 
Unlike many learned optimizers that struggle with cost, stability or show only a 
~1.2–1.3x speedup<d-footnote>While a ~1.2–1.3x speedup is remarkable, in practice it usually does not justify the immense amount of extra work (e.g. efficient distributed implementation, tuning, potential instabilities especially in mixed/low-bit precision, investigating unexpected side effects like overfitting or poor generalization) that is required for actual large-scale usefulness.</d-footnote>, 
...
[Celo](https://arxiv.org/abs/2501.12670) and [μLO](https://arxiv.org/abs/2406.00153) **that will be presented at ICLR 2026 🎉🎉🎉**,
make a significant step in improving learned optimizers and making them more cost-effective and stable (i.e. without big loss spikes) to train and use.

<div class="row mt-3 l-body figure-container-desktop">
   <div class="col-sm mt-3 mt-md-0" style="text-align: center;">
        {% include figure.liquid path="assets/img/2026-02-19-nino/neural_graph_llama.png" class="img-fluid d-block mx-auto rounded z-depth-1" zoomable=true %}
    </div> 
</div>
<div class="caption" style="text-align: center; margin-top: -35px; margin-bottom: 25px;">
